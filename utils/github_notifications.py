...
class GitHubNotificationHandler:
        ...
        try:
            ...
            notification_row = [
                ...
                json.dumps(notification_data,
                           ensure_ascii=False),  # dados_json
                'pendente'                                    # status
            ]

            # Adiciona à planilha matriz
            matrix_uploader = GoogleDriveUploader(is_matrix=True)
            matrix_uploader.append_data_to_sheet(
                'notificacoes_pendentes', [notification_row])
    ...
    def trigger_notification_workflow(self, notification_type: str, recipient_email: str,
        ...
        if success:
            logger.info(
                f"Notificação '{notification_type}' adicionada à fila para processamento automático")
        else:
            logger.error(
...
def notify_new_access_request(admin_email: str, user_email: str, user_name: str, justification: str = ""):
    ...
    return handler.trigger_notification_workflow(
        ...
        admin_panel_url=st.secrets.get("app", {}).get(
            "url", "https://isnpecoessmaia.streamlit.app")
    )
...
def send_trial_expiration_notifications():
    ...
    try:
        ...
        trial_users['trial_end_date'] = pd.to_datetime(
            trial_users['trial_end_date']).dt.date
        ...
        for _, user in expiring_soon.iterrows():
            ...
            logger.info(
                f"Notificação de trial expirando enviada para {user['email']}")
        ...
        for _, user in expiring_tomorrow.iterrows():
            ...
            logger.info(
                f"Notificação de trial expirando (1 dia) enviada para {user['email']}")
...
def create_notifications_sheet_if_not_exists():
    ...
    try:
        ...
        # Adiciona cabeçalhos
        matrix_uploader.append_data_to_sheet(
            'notificacoes_pendentes', [headers])
