        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    st.warning(
                        f"⚠️ Erro na chave atual. Tentando com outra chave... ({attempt + 1}/{max_retries})")
                    ...
                    st.error(
                        f"❌ Erro após {max_retries} tentativas com diferentes chaves: {error_msg}")

                    # Mostra estatísticas
                    stats = self.key_manager.get_statistics()
                    st.warning(
                        f"📊 Chaves disponíveis: {stats['available_keys']}/{stats['total_keys']}")
        ...
        for attempt in range(max_retries):
            try:
                with st.spinner(f"🤖 Analisando '{pdf_file.name}' com IA (tentativa {attempt + 1})..."):
                    ...
                    part_pdf = {"mime_type": "application/pdf",
                                "data": pdf_bytes}
                    ...
                    st.success(
                        f"✅ Dados extraídos com sucesso de '{pdf_file.name}'!")
                    return extracted_data

            except json.JSONDecodeError as je:
                st.error("❌ A IA não retornou JSON válido")
                st.text_area("Resposta recebida:",
                             value=response.text, height=150)
                return None

            except Exception as e:
                error_msg = str(e)
                logger.error(
                    f"Tentativa {attempt + 1}/{max_retries} falhou na extração: {error_msg}")
                ...
                if attempt < max_retries - 1:
                    st.warning(
                        f"⚠️ Erro na chave atual. Tentando novamente... ({attempt + 1}/{max_retries})")
                    time.sleep(retry_delay)
