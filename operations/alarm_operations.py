...
def save_alarm_inspection(system_id, overall_status, results_dict, photo_file, inspector_name):
    ...
    try:
        ...
        if photo_file:
            ...
            try:
                ...
                if photo_link:
                    ...
                    st.warning("⚠️ Não foi possível salvar a foto. Continuando sem foto...")
                    
            except Exception as photo_error:
                st.warning(f"⚠️ Erro no upload da foto: {photo_error}. Continuando sem foto...")
                photo_link = None
        ...
        try:
            ...
            st.warning(f"⚠️ Erro ao converter resultados para JSON: {json_error}")
        ...
        import logging
        logging.info(f"Inspeção de alarme salva: {log_details}")
...
def save_alarm_action_log(system_id, problem, action_taken, responsible, photo_file=None):
    try:
        ...
        photo_link = None
        if photo_file:
            photo_link = upload_evidence_photo(photo_file, system_id, "acao_corretiva_alarme")
...
def generate_alarm_action_plan(non_conformities):
    ...
    if critical_issues:
        ...
        return f"⚠️ CORREÇÕES NECESSÁRIAS: Corrigir {len(other_issues)} não conformidade(s) identificada(s): {', '.join(other_issues[:2])}{'...' if len(other_issues) > 2 else ''}."
