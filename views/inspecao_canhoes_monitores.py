    ...
    with tab_inspection:
        ...
        if not can_edit():
            ...
            if df_inventory.empty:
                ...
                equipment_options = ["Selecione..."] + sorted(df_inventory['id_equipamento'].tolist())
                selected_id = st.selectbox("Selecione o Canhão Monitor", equipment_options)

                if selected_id != "Selecione...":
                    equip_data = df_inventory[df_inventory['id_equipamento'] == selected_id].iloc[0]
                    st.info(f"**Localização:** {equip_data.get('localizacao', 'N/A')} | **Marca/Modelo:** {equip_data.get('marca', 'N/A')} / {equip_data.get('modelo', 'N/A')}")
                    ...
                    with st.form(key=f"inspection_form_{selected_id}"):
                        ...
                        for category, questions in CHECKLIST_VISUAL.items():
                            ...
                            for question in questions:
                                key = f"{selected_id}_{question}".replace(" ", "_")
                                answer = st.radio(question, ["Conforme", "Não Conforme", "N/A"], key=key, horizontal=True)
                        ...
                        if inspection_type == "Teste Funcional (Anual)":
                            ...
                            for category, questions in CHECKLIST_FUNCIONAL.items():
                                ...
                                for question in questions:
                                    key = f"{selected_id}_{question}".replace(" ", "_")
                                    answer = st.radio(question, ["Aprovado", "Reprovado", "N/A"], key=key, horizontal=True)
                                    inspection_results[question] = answer
                                    if answer == "Reprovado":
                                        has_issues = True
                        ...
                        photo_file = None
                        if has_issues:
                            st.warning("Uma ou mais não conformidades foram encontradas. Anexe uma foto como evidência.")
                        ...
                        if submitted:
                            if has_issues and not photo_file:
                                ...
                                with st.spinner("Salvando..."):
                                    ...
                                    if success:
                                        st.success(f"Registro para '{selected_id}' salvo com sucesso!")
                                        if not has_issues: st.balloons()
                                        st.cache_data.clear()
                                    else:
    ...
    with tab_register:
        ...
        if not can_edit():
            ...
            with st.form("new_canhao_form", clear_on_submit=True):
                st.info("Preencha os dados do novo equipamento a ser adicionado ao inventário.")
                col1, col2 = st.columns(2)
                new_id = col1.text_input("**ID do Equipamento (Obrigatório)**", help="Use um código único, ex: CM-01")
                new_location = col2.text_input("**Localização (Obrigatório)**", help="Descrição do local, ex: Dique do Tanque TQ-101")
                    ...
                    if not new_id or not new_location:
                        ...
                        with st.spinner("Cadastrando..."):
                            if save_new_canhao_monitor(new_id, new_location, new_brand, new_model):
                                st.success(f"Canhão Monitor '{new_id}' cadastrado com sucesso!")
                                st.cache_data.clear()
